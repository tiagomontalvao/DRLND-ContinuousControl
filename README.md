...
The goal of this project is to train an agent to control a robotic arm to follow a moving target, using a Deep Deterministic Policy Gradient (DDPG).  
...
![Trained Agent](img/trained_agent.gif "Trained Agent")
...
All the training procedures are inside `Continuous_Control.ipynb`. Run all the cells to train the agent, or just run the imports cell and the Section 5, which loads trained weights into a new agent.
